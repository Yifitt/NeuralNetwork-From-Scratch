...
public:
    ...
    void save_model(const std::string& filename) {
        try {
            ...
            j["num_layers"] = layers.size();

            // Architecture info
            std::vector<int> architecture;
            for (const auto& layer : layers) {
                nlohmann::json layer_json = layer.to_json();
                if (architecture.empty()) {
                    architecture.push_back(layer_json["input_size"]);
                }
                architecture.push_back(layer_json["output_size"]);
            }
